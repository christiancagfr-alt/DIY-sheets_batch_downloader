...
def default_credentials_path():
    preferred = os.path.join(app_base_dir(), "青苹果下载器 1.json")
    if os.path.exists(preferred):
        return preferred
    preferred = os.path.join(bundle_base_dir(), "青苹果下载器 1.json")
    if os.path.exists(preferred):
        return preferred
...
class DownloadWorker(WorkerBase):
    ...
    def __init__(self, credentials_path, token_path, settings, output_dir, skip_existing, backfill_enabled, backfill_col):
        super().__init__(credentials_path, token_path)
        self.settings = settings
...
class MainWindow(QMainWindow):
    ...
    def build_ui(self):
        ...
        self.keyword_edit.setPlaceholderText("例如：刘乐 或 刘乐-刘平")
    ...
    def show_usage_guide(self):
        QMessageBox.information(
            ...
            "4. 可在“只下载包含”输入人名，例如：刘乐 或 刘乐-刘平。\n"
            "5. 先点“预览读取”确认，再点“开始下载”。\n"
            "6. 多个保存方案可以点“执行所有方案”按顺序自动下载。"
        )
